...
Keep these generated app env files private as well: `.gitignore` does not prevent cloud-sync software from syncing them. Real accounts are now the default. Fixture impersonation additionally requires `EMBER_AUTH_MODE=local` in both apps; the helper does not select it automatically. Local actor access is a development-only bridge, not production authentication; never expose this configuration publicly. Leave `NODE_ENV` in development when running the local servers, and restart them after configuration changes.
